...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="card">
        <form method="POST" action="{{ route('operator.vehicles.store') }}" enctype="multipart/form-data" class="p-6 space-y-6" onsubmit="return confirmAndSubmit(event)">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div id="kedudukan_detail_container" style="display: {{ old('kedudukan') ? 'block' : 'none' }};" class="md:col-span-2">
                        ...
                        <input type="text" name="kedudukan_detail" id="kedudukan_detail" value="{{ old('kedudukan_detail') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                    ...
                    (function(){
                        ...
                        function update(v){
                            console.log('[kedudukan] update called, value=', v, 'sel=', sel, 'cont=', cont, 'inp=', inp);
                            if(v){
                        ...
                        if(sel){ sel.addEventListener('change', function(){ update(this.value); }); update(sel.value || ''); } else { console.log('[kedudukan] select element not found'); }
                    })();
                    </script>

                    <script>
                    // global helper similar to document_status behavior
                    window.toggleKedudukan = function(v){
                        var cont = document.getElementById('kedudukan_detail_container');
                        var inp = document.getElementById('kedudukan_detail');
                        if(v){ if(cont) cont.style.display = 'block'; if(inp){ inp.setAttribute('required','required'); if(v==='BMN') inp.placeholder='Nomor BMN atau Tahun BMN'; else if(v==='Sewa') inp.placeholder='Nama perusahaan penyewa'; else inp.placeholder='Keterangan lain'; } }
                    ...
                    <div>
                        ...
                        <input type="text" name="bpkb_number" id="bpkb_number" value="{{ old('bpkb_number') }}" required maxlength="100"
                    ...
                    <div>
                        ...
                        <input type="text" name="chassis_number" id="chassis_number" value="{{ old('chassis_number') }}" required maxlength="100"
                    ...
                    <div>
                        ...
                        <input type="text" name="engine_number" id="engine_number" value="{{ old('engine_number') }}" required maxlength="100"
                    ...
                    <div>
                        ...
                        <input type="number" name="cc_amount" id="cc_amount" value="{{ old('cc_amount') }}" required min="50" max="10000"
                ...
                <div id="photo-preview-filename" class="text-xs text-gray-700 mt-1"></div>
                        ...
                        <script>
                        // Admin-style camera modal setup
                        (function(){
                            ...
                            if(openCameraBtn){
                                openCameraBtn.addEventListener('click', async function(){
                                    ...
                                    try{
                                        ...
                                        // if camera cannot be accessed (permission denied, device error), fall back to file picker
                                        return fallbackToFileInput();
                                    }
                                });
                            }
                            ...
                            if(switchCameraBtn) switchCameraBtn.addEventListener('click', async function(){
                                currentFacingMode = currentFacingMode === 'environment' ? 'user' : 'environment';
                                try{ stopCamera(); await startCamera(); }catch(e){ console.error(e); }
                            });
                            ...
                            function addCapturedPhotoToInput(){
                                ...
                                fetch(dataURL).then(res=>res.blob()).then(blob=>{
                                    ...
                                    // Replace any existing file with the newly captured file
...
<!-- Confirmation Modal (same as admin) -->
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full transform transition-all">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center">
                ...
                <h3 class="ml-3 text-lg font-medium text-gray-900">Konfirmasi Tambahkan Kendaraan</h3>
            </div>
        </div>
        <div class="px-6 py-4">
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
            ...
            <button type="button" onclick="submitForm()" class="px-4 py-2 btn btn-primary">Ya, Tambah</button>
        </div>
    </div>
</div>
...
function confirmAndSubmit(e) {
    e.preventDefault();
    // populate modal
    document.getElementById('modal-brand').innerText = document.getElementById('brand').value || '-';
    document.getElementById('modal-model').innerText = document.getElementById('model').value || '-';
    document.getElementById('modal-license').innerText = document.getElementById('license_plate').value || '-';
    document.getElementById('modal-year').innerText = document.getElementById('year').value || '-';
...
// show/hide document notes
document.addEventListener('DOMContentLoaded', function(){
    var docStatus = document.getElementById('document_status');
    if(docStatus){
        docStatus.addEventListener('change', function(){
            var c = document.getElementById('document_notes_container');
            c.style.display = this.value === 'tidak_lengkap' ? 'block' : 'none';
        });
    }
});
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(kedSelect){
        ...
        if(kedSelect.value){
            if(kedContainer) kedContainer.style.display = 'block';
            if(kedInput) kedInput.setAttribute('required','required');
        } else {
            if(kedContainer) kedContainer.style.display = 'none';
            if(kedInput) kedInput.removeAttribute('required');
        }

        kedSelect.addEventListener('change', function(){
            if(this.value){
                if(kedContainer) kedContainer.style.display = 'block';
                if(kedInput) kedInput.setAttribute('required','required');
            } else {
                if(kedContainer) kedContainer.style.display = 'none';
                if(kedInput){ kedInput.removeAttribute('required'); kedInput.value = ''; }
            }
        });
    }
});
...
// Photo preview, upload and drag-drop handlers (guarded)
document.addEventListener('DOMContentLoaded', function() {
    const photoInput = document.getElementById('photo');
